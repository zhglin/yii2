...
class BaseUrl
{
    ...
    {
        $route = (array) $route;
        $route[0] = static::normalizeRoute($route[0]); //标准化route

        if ($scheme !== false) {
            return static::getUrlManager()->createAbsoluteUrl($route, is_string($scheme) ? $scheme : null); //绝对地址
        } else {
     ...
     * @throws InvalidParamException a relative route is given while there is no active controller
     * 标准化$route 如果开头以/是绝对地址 直接返回
     * 否则是相对地址 $route前面加上当前的model 或者 controller
     */
    protected static function normalizeRoute($route)
    {
        $route = Yii::getAlias((string) $route); //别名 alias "@posts" is defined as "/post/index"  Url::to(['@posts']);
        if (strncmp($route, '/', 1) === 0) { // /开头 是个绝对的地址
            // absolute route
            return ltrim($route, '/');
        }
        /*
         *  相对地址
         *  current module is admin and the current controller is post,
         * Url::to(['post/index']);   /index.php?r=admin%2Fpost%2Findex  相对于当前的model
         * Url::to(['index']);  /index.php?r=admin%2Fpost%2Findex  相对于当前的model controller
         * Url::to(['']);   //就是当前的地址
         */
        // relative route
        ...
        if (strpos($route, '/') === false) { //不含有/  $route只是action  返回当前地址的 controller/$route
            // empty or an action ID   route = null 返回当前的route 否则返回 当前的controller/$route
            return $route === '' ? Yii::$app->controller->getRoute() : Yii::$app->controller->getUniqueId() . '/' . $route;
        } else { // 含有/返回 当前的module/$route/  $route是controller/action
            // relative to module
